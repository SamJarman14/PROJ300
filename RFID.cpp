...
vector<string> rfid_tags;                   // Stores ordered list of tag ID's and names 
...
void process_tag(bool isAdding) {
    ...
    // Timeout timer for when waiting for tag to be scanned (added or removed)
    Timer timer;
    timer.start();
    ...
    while(true){
        if (serial_port.readable()) {
            ...
            if (Byte > 0) {
                ...
                while (curr_Byte < 12) {    // Ensure exactly 12 bytes are read
                    if (serial_port.readable()) { 
                ...
                if ((match == true) && (skip_match_check == false)){
                    ...
                    esp32.read(&entry, 1);           // Read feedback from the ESP32
